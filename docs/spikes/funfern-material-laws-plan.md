...
**Adopted architecture — implementation in progress:** use integrated nodal primary flux `Q`, independent two-component complementary flux `b` at the six element quadrature points, and explicitly physical trace/oscillator memory. There is no production bulk potential, potential gauge rebasing, or seam-offset solve. Authoritative mechanical `s₀`, named electric/magnetic loss channels, analytic legacy-source migration, phase anchors, frozen-rate passive loss, value-continuous ramp reversal, and recoverable global acceptance are agreed. The [core spike](funfern-material-laws-spike-report.md) justifies direct state over potential-plus-loss memory. Stages 0–6 are complete: contracts and inert authoring are frozen; the f64 direct-state CPU reference composes linear sources, physical losses, thin gaps, both outgoing orders, the paired filter, and bounded scalar/vector/physical-history transfer; the production f32 GPU core closes linear parity and steady performance in the [Stage 4 report](funfern-material-laws-stage4-report.md); latest-state transfer, physical-history/runtime handoff, paused events and rollback close in the [Stage 5 report](funfern-material-laws-stage5-report.md); and direct-state rendering, probes, energy/flow/far-field, static-linear AMR and the shared-skin application cutover close in the [Stage 6 report](funfern-material-laws-stage6-report.md). Authored non-inert laws remain explicitly non-executable.
...
The CPU prepares geometry, law metadata, local transfer stencils, trace correspondence, and factorizations while the old accepted state evolves. At commit, GPU field-dependent calculations use the latest accepted state. Target upload and asynchronous admission must not extend that snapshot into a display pause: record the exact transferred absolute/local clock, keep the accepted source live while admission is pending, and let the admitted target consume the short queued-step backlog from that clock before its first new display state. Rejection retains the continuing source. Keep generation validation/rollback and coherent display switching. Additional ordered GPU dispatches are allowed; an additional live CPU field-readback/calculation/upload cycle is not.
...
The bounded spike work is complete, and Stages 1–6 have implemented its accepted
linear contracts. The canonical direct-state core and its synchronized consumers
are now the connected production path.
...
The user accepted the candidate's reflection/nonlocal-cost tradeoff. Do not reopen the bulk representation or repeat broad spikes without new contrary evidence. Stages 3–6 close the declared linear irregular-remap, physical-history, f32, actual-core transfer and migrated-consumer gates. Exact curved DtN accuracy is not claimed; time-driven and nonlinear composition remain open at their stated stages.
...
| 9 (complete) | GPU nonlinear execution, admission/recovery and actual-core performance | Kerr/saturable on the device with every composition but the grid filter, parity, failure/recovery and cost in the [Stage 9 report](funfern-material-laws-stage9-report.md) |
| 10 (complete) | Complete Simplified/Advanced UI, presets, demonstrations and compatibility docs | Second release closed in the [Stage 10 report](funfern-material-laws-stage10-report.md): the grid filter and area readout on every supported composition, an amplitude-aware size rule, the regrouped editor, four measured gallery scenes, and five defects found and fixed on the way (device clock, device loss, named loss in the app, the reflectionless pairing, the Mechanical formula) |
...
the native application integration test covers worker return. The threaded WASM
bundle rebuilds `std` with atomics and starts exactly one shared-memory Rayon Web
Worker behind COOP/COEP; Chrome integration asserts both cross-origin isolation
and actual worker activation while the complete WebGPU application advances and
resizes. GitHub Pages receives a separate stable-toolchain bundle that retains the
cooperative four-millisecond runner because Pages cannot provide those headers;
...
**Status as of 24 September 2026:** Stages 0–10 are complete; the [Stage 10 report](funfern-material-laws-stage10-report.md) closes the second release boundary and lists what stays refused or open, including the characterized long-run device drift on driven and nonlinear scenes; the [Stage 9 report](funfern-material-laws-stage9-report.md) records the device port and application enablement; the [Stage 8 report](funfern-material-laws-stage8-report.md) records the CPU nonlinear reference, the three Stage 7 defects and one harness defect found and fixed during it, and what Stage 9 inherits. The [Stage 7 report](funfern-material-laws-stage7-report.md) supersedes the Stage 7 status paragraphs below, which predate application enablement, mixed static/driven handoffs and AMR. It lists the three defects to fix before Stage 8 begins.
