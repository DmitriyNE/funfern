...
Stage 3 adds the CPU source, boundary, loss, filter and transfer composition to the linear direct reference. The old production solver remains the comparison and application path until the later cutover stages. Each stage produces a reviewable change with tests and an engineering-log entry. Keep implementation status distinct from planned behavior, and keep unsupported features unavailable rather than silently ignored.
