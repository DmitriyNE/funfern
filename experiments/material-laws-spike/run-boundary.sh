...
spike_tmp="$(mktemp -d /tmp/funfern-boundary-spike.XXXXXX)"
...
"$spike_tmp/export" --curved > "$spike_tmp/fixtures.json"
OPENBLAS_NUM_THREADS=1 VECLIB_MAXIMUM_THREADS=1 PYTHONDONTWRITEBYTECODE=1 \
  python3 "$spike_dir/boundary.py" "$spike_tmp/fixtures.json"
