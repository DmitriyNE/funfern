...
// Mirrors the shared block in canonical_probe.wgsl. This shader is at the
// portable eight-binding limit and cannot reach the law tables, so it carries
// the same arithmetic over its own structures rather than sharing the text.
...
fn sample_area_elements(@builtin(global_invocation_id) invocation: vec3<u32>) {
    ...
    let shares_a = contribution.node_shares_a;
    let shares_b = contribution.node_shares_b;
    let nodal = vec4<f32>(
        accepted_q(a.x) * accepted_q(a.x) * nodes[a.x].mass_loss.y,
        accepted_q(a.y) * accepted_q(a.y) * nodes[a.y].mass_loss.y,
        accepted_q(a.z) * accepted_q(a.z) * nodes[a.z].mass_loss.y,
        accepted_q(a.w) * accepted_q(a.w) * nodes[a.w].mass_loss.y);
    let nodal_tail = vec4<f32>(
        accepted_q(b.x) * accepted_q(b.x) * nodes[b.x].mass_loss.y,
        accepted_q(b.y) * accepted_q(b.y) * nodes[b.y].mass_loss.y,
        accepted_q(b.z) * accepted_q(b.z) * nodes[b.z].mass_loss.y, 0.0);
    var energy = 0.5 * (dot(nodal, shares_a)
        + dot(nodal_tail, vec4<f32>(shares_b.xyz, 0.0)));
    for (var local = 0u; local < 6u; local += 1u) {
        energy += 0.5 * contribution.sample_inverse[local].w
            * dot(accepted_b(start + local), sample_field[local]);
    }
    accumulated.w = shares_b.w * energy;
