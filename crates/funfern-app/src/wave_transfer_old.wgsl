...
fn source_acceleration(i: u32) -> f32 {
    ...
    return forcing.source.position_width_enabled.w
...
fn transfer(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    var mapped_reconstruction_a = 0.0;
    var mapped_reconstruction_b = 0.0;
    ...
    transfers[i].mapped = vec4<f32>(mapped_velocity, current, mapped_auxiliary, mapped_reconstruction_a);
    transfers[i].auxiliary.y = mapped_reconstruction_b;
    if i == 0u {
